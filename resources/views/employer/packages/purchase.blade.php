    ...
    <div class="row gx-5 gy-4">
      ...
      <div class="card border-0 shadow-sm h-100 rounded-4">
        ...
        <form id="paymentForm" action="{{ route('employer.packages.subscribe', $package->id) }}" method="POST"
          ...
          <label
          class="d-flex flex-column align-items-center p-3 bg-white border rounded-4 hover-shadow text-center"
          style="cursor:pointer;">
          <input class="form-check-input mb-2" type="radio" name="bank" value="{{ $bank->id }}" {{ $loop->first ? 'checked' : '' }}>
