...
@section('content')
                ...
                @foreach($company->branches as $branch)
                    ...
                    <div class="modal fade" id="editBranchModal{{ $branch->id }}" tabindex="-1">
                        <div class="modal-dialog">
                            <form method="POST" action="{{ route('employer.branches.update', $branch->id) }}">
                                @csrf
                                @method('PUT')
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <select name="city_id" class="form-select select2-city"
                                                data-parent="#editBranchModal{{ $branch->id }}">
                                                <option value="">-- Chọn thành phố --</option>
                                                @foreach($locations as $loc)
                                                    <option value="{{ $loc->id }}" {{ old('city_id', $branch->city_id) == $loc->id ? 'selected' : '' }}>
                                                        {{ $loc->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                    ...
                    <div class="modal fade" id="deleteBranchModal{{ $branch->id }}" tabindex="-1">
                        <div class="modal-dialog">
                            <form method="POST" action="{{ route('employer.branches.destroy', $branch->id) }}">
                                @csrf
                                @method('DELETE')
                                <div class="modal-content">
    ...
    <div class="modal fade" id="createBranchModal" tabindex="-1">
        <div class="modal-dialog">
            <form method="POST" action="{{ route('employer.branches.store') }}">
                @csrf
                <div class="modal-content">
