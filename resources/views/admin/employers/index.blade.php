...
@section('content')
    @php
        /** Ép mọi kiểu về chuỗi an toàn để in ra HTML */
        $toText = function ($value, $fallback = '-') {
            ...
            if ($value instanceof \Illuminate\Support\Collection) {
                $arr = $value->all();
                return implode(', ', array_map(fn($v) => is_scalar($v) ? (string) $v : (is_array($v) ? json_encode($v, JSON_UNESCAPED_UNICODE) : (string) $v), $arr));
            }
            if (is_array($value)) {
                return implode(', ', array_map(fn($v) => is_scalar($v) ? (string) $v : (is_array($v) ? json_encode($v, JSON_UNESCAPED_UNICODE) : (string) $v), $value));
            }
        ...
        /** Status → text + class, luôn an toàn */
        $statusText = function ($value) use ($toText) {
            $str = $toText($value, '-');
            ...
            $classSolid = match ($raw) {
                'active' => 'bg-success',
                'inactive' => 'bg-secondary',
                'banned' => 'bg-danger',
                default => 'bg-secondary',
            };
            return [$str, $classPill, $classSolid];
        };
    @endphp
            ...
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    @forelse ($employers as $employer)
                        @php
                            [$empStatusStr, $empStatusPillClass, $empStatusSolidClass] = $statusText($employer->status);
                        @endphp
                        <tr>
                            <td class="text-start">{{ $toText($employer->email) }}</td>
                            <td class="fw-semibold text-start">{{ $toText($employer->name) }}</td>
                            <td class="text-start">{{ $toText($employer->phone_number) }}</td>
                            <td class="text-center">
                                <span class="badge rounded-pill px-3 py-2 fw-medium {{ $empStatusPillClass }}">
                                    {{ $empStatusStr }}
                                </span>
                            </td>
                            <td class="text-center">
                            ...
                            <div class="modal-dialog modal-lg modal-dialog-centered">
                                <div class="modal-content border-0 shadow">
                                    ...
                                    <div class="modal-body">
                                        <p><strong>Email:</strong> {{ $toText($employer->email) }}</p>
                                        <p><strong>Tên:</strong> {{ $toText($employer->name) }}</p>
                                        <p><strong>Số điện thoại:</strong> {{ $toText($employer->phone_number) }}</p>
                                        <p><strong>Trạng thái:</strong>
                                            <span class="badge {{ $empStatusSolidClass }}">{{ $empStatusStr }}</span>
                                        </p>
                        ...
                        <div class="modal fade" id="deleteModal{{ $employer->id }}" tabindex="-1"
                            aria-labelledby="deleteModalLabel{{ $employer->id }}" aria-hidden="true">
    ...
    @foreach ($employers as $employer)
        @foreach ($employer->companies as $company)
            @php
                [$compStatusStr] = $statusText($company->status);
            @endphp
            <div class="modal fade" id="showCompanyModal{{ $company->id }}" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog modal-lg modal-dialog-centered">
                    <div class="modal-content border-0 shadow">
                        <div class="modal-header bg-primary text-white">
                            <h5 class="modal-title">Chi tiết công ty: {{ $toText($company->name) }}</h5>
                            <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                        </div>
                        <div class="modal-body">
                            <p><strong>Tên công ty:</strong> {{ $toText($company->name) }}</p>
                            <p><strong>Email:</strong> {{ $toText($company->email) }}</p>
                            <p><strong>Địa chỉ:</strong> {{ $toText($company->address) }}</p>
                            <p><strong>Số điện thoại:</strong> {{ $toText($company->phone) }}</p>
                            <p><strong>Website:</strong>
                                @if (!empty($company->website))
                                    <a href="{{ $toText($company->website) }}" target="_blank">{{ $toText($company->website) }}</a>
                                @else
                                    -
                                @endif
                            </p>
                            <p><strong>Mô tả:</strong> {!! nl2br(e($toText($company->description))) !!}</p>
                            <p><strong>Lợi ích:</strong> {!! nl2br(e($toText($company->benefits))) !!}</p>
                            <p><strong>Năm thành lập:</strong> {{ $toText($company->founded_year) }}</p>
                            <p><strong>Quy mô công ty:</strong> {{ $toText($company->company_size) }}</p>
                            <p><strong>Ngành nghề:</strong> {{ $toText($company->industry) }}</p>
                            <p><strong>Trạng thái:</strong> {{ $compStatusStr }}</p>
                            <p><strong>Ngày tạo:</strong> {{ $toText(optional($company->created_at)) }}</p>
                        </div>
