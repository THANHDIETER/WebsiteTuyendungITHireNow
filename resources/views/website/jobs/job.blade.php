  ...
  <div class="container">
    <div class="row g-4">
      ...
      <div class="col-md-6 col-lg-4">
        <div class="job-card shadow-sm rounded p-4 h-100 position-relative">
          @if($job->is_featured)
          <div class="featured-ribbon">🌟 NỔI BẬT</div>
          @endif

          <a href="{{ route('jobs.show', $job->slug) }}" class="d-block mb-3 overflow-hidden rounded" style="height:160px;">
          ...
          <h5 class="job-title mb-2 text-truncate" title="{{ $job->title }}">
            <a href="{{ route('jobs.show', $job->slug) }}" class="text-dark text-decoration-none">{{ $job->title ?: 'Không có tiêu đề' }}</a>
          </h5>

          <p class="text-success fw-semibold mb-2 small">{{ $job->jobType->name ?? ucfirst($job->job_type ?? 'N/A') }}</p>

          <p class="text-muted small mb-3" style="min-height: 65px;">{!! $job->description ? Str::limit(strip_tags($job->description), 110) : 'Chưa có mô tả' !!}</p>
          ...
          <div class="d-flex justify-content-between align-items-center">
            ...
            <a href="{{ route('jobs.show', $job->slug) }}" class="btn btn-primary btn-sm rounded-pill px-4 py-2">Ứng tuyển</a>
          </div>
        </div>
      </div>
      @empty
