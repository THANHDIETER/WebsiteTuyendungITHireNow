...
@section('content')
    ...
    <div class="page-header-area d-flex justify-content-center align-items-center text-center"
        ...
        role="img" aria-label="Job details banner" loading="lazy">
                        ...
                        <div class="p-3 p-md-4">
                            <div class="table-responsive">
                                <table class="table table-sm align-middle mb-0">
                                    <tbody>
                                    <tr>
                                        <td class="text-muted small"><i class="bi bi-briefcase-fill me-1"></i> Loại công việc</td>
                                        <td class="text-end">{{ optional($job->jobType)->name ?? '-' }}</td>
                                    </tr>
                                    <tr>
                                        <td class="text-muted small"><i class="bi bi-cash-stack me-1"></i> Mức lương</td>
                                        <td class="text-end">
                                            @if ($hasMin && $hasMax)
                                                {{ number_format($job->salary_min) }} - {{ number_format($job->salary_max) }} {{ $currency }}
                                            @elseif ($hasMin)
                                    ...
                                    <tr>
                                        <td class="text-muted small"><i class="bi bi-geo-alt-fill me-1"></i> Địa chỉ</td>
                                        <td class="text-end">{{ $job->address ?: '-' }}</td>
                                    </tr>
                                    <tr>
                                        <td class="text-muted small"><i class="bi bi-laptop me-1"></i> Chính sách làm việc từ xa</td>
                                        <td class="text-end">{{ optional($job->remotePolicy)->name ?? '-' }}</td>
                                    </tr>
                                    <tr>
                                        <td class="text-muted small"><i class="bi bi-bar-chart-steps me-1"></i> Cấp bậc</td>
                                        <td class="text-end">{{ optional($job->level)->name ?? '-' }}</td>
                                    </tr>
                                    <tr>
                                        <td class="text-muted small"><i class="bi bi-person-workspace me-1"></i> Kinh nghiệm</td>
                                        <td class="text-end">{{ optional($job->experience)->name ?? '-' }}</td>
                                    </tr>
                                    <tr>
                                        <td class="text-muted small"><i class="bi bi-translate me-1"></i> Ngôn ngữ</td>
                                        <td class="text-end">{{ optional($job->language)->name ?? '-' }}</td>
                                    </tr>
                                    <tr>
                                        <td class="text-muted small"><i class="bi bi-calendar-date me-1"></i> Ngày đăng</td>
                                        <td class="text-end">{{ $job->created_at ? $job->created_at->format('d/m/Y') : '-' }}</td>
                                    </tr>
                                    <tr>
                                        <td class="text-muted small"><i class="bi bi-calendar-event me-1"></i> Hạn nộp hồ sơ</td>
                                        <td class="text-end">{{ $job->deadline ? $job->deadline->format('d/m/Y') : '-' }}</td>
                                    </tr>
                                    </tbody>
                                </table>
                            </div>
