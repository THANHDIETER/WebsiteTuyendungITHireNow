...
@media (max-width: 600px) {
    #typing-indicator { font-size: 14px; margin-left: 2px;}
    .typing-avatar { width: 18px; height: 18px;}
}
</style>
@endpush
                    ...
                    @forelse ($conversations as $conversation)
                        @php
                            ...
                            $lastMsgTime = optional($conversation->latestMessage)->created_at
                                ...
                                : 'Chưa có tin nhắn';
                        @endphp
                        <li class="list-group-item p-2 {{ $isActive ? 'bg-light border-start border-4 border-primary' : '' }}">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    messageInput?.addEventListener('input', function() {
        ...
        if (Date.now() - lastTyped > 1200) {
            fetch("{{ route('chat.typing', $conversationId) }}", {
                method: 'POST',
                headers: {
